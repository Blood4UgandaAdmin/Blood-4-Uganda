...
if(strlen($password) < 8 ) {
    header("Location:../admin/failure.php?id=$id&error=Password must be at least 8 characters!");
		...
		while($row = mysqli_fetch_array($resultmax)){
			...
			$sqlid = "SELECT `LTID` AS max FROM `labtechnician` ORDER BY `LTID` DESC";
			$resultmax = mysqli_query($conn, $sqlid); 
			//while($row = mysqli_fetch_array($resultmax)){
			...
			$sql3 = "INSERT INTO labtechnician (LTID, sname, fname,dob, bloodGroup, phoneCode, tel, email, country, region, district, plotNo,RegisteredBy)
						VALUES
