...
if(strlen($password) < 8 ) {
    header("Location:../admin/failure.php?id=$id&error=Password must be at least 8 characters!");
		...
		while($row = mysqli_fetch_array($resultmax)){
			...
			$sqlid = "SELECT `VID` AS max FROM `volunteer` ORDER BY `VID` DESC";
			$resultmax = mysqli_query($conn, $sqlid); 
			//while($row = mysqli_fetch_array($resultmax)){
			...
			//inert into the labtechnician table
			$sql3 = "INSERT INTO volunteer (VID, sname, fname,dob, bloodGroup,gender, phoneCode, tel, email, country, region, district, plotNo, bloodBank,RegisteredBy)
						VALUES
						('$userID','$sname','$fname','$dob','$bloodGroup','$gender','$phoneCode','$tel','$email','$country','$region','$district','$plotNo', '$bloodBank','$id')";
