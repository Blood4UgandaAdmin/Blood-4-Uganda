    ...
    header("Location:../admin/failure.php?id=$id&error=Password must be at least 8 characters!");
		...
		while($row = mysqli_fetch_array($resultmax)){
			...
			//while($row = mysqli_fetch_array($resultmax)){
				...
				$nuserid = $row['max']; 
				$URSID = intval(substr($nuserid , 2)) + 1; 
						$URSID = uniqid();	
						if($URSID<10){
							$URSID = 'URS000'.$URSID;
						}
				...
				VALUES
				('$notificationid','$URSID', '$messages', '$id')";
			//insert into the users table
			$sql2 = "INSERT INTO users (userID, password, username,email, roles) 
						VALUES 
						('$URSID','$password','$username','$email','$roles')";
						
			//inert into the bloodbanks table
			$sql3 = "INSERT INTO urs (URSID, fullname,phoneCode, tel, email, country, region, district, plotNo,RegisteredBy)
						VALUES
						('$URSID','$fname','$phoneCode','$tel','$email','$country','$region','$district','$plotNo','$id')";
